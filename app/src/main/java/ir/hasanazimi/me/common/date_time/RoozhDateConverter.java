...
public class RoozhDateConverter {
    ...
    public String toString(Boolean strMonth) {
        if (strMonth)
            return String.format("%02d %s %04d", getDay(), mounths[getMonth() - 1],getYear() );
        else
            return toString();
    }
    ...
    public void gregorianToPersian(Date date) {
        ...
        int jd = jG2JD(calender.get(Calendar.YEAR), calender.get(Calendar.MONTH),calender.get(Calendar.DAY_OF_MONTH), 0);
        jD2Jal(jd);
        this.year = jY;
        this.month = jM;
        this.day = jD;
